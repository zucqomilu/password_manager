...
def save_session(username: str, key: bytes):
    ...
    with open(src.constants.get_session(), "w") as f:
        json.dump(session_data, f)
        logger.info(f"Session saved for user: {username}")

def load_session() -> tuple[str, Fernet] | None:
    if not os.path.exists(src.constants.get_session()):
        logger.info("No session found.")
        return None

    with open(src.constants.get_session(), "r") as f:
        try:
...
def clear_session():
    if os.path.exists(src.constants.get_session()):
        os.remove(src.constants.get_session())
